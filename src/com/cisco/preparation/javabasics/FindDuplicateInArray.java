...
public class FindDuplicateInArray {
	public static void main(String[] args) {
		// int[] arr = { 3, 2, 1, 4, 1 };
		int[] arr = { 247, 240, 303, 9, 304, 105, 44, 204, 291, 26, 242, 2, 358, 264, 176, 289, 196, 329, 189, 102, 45,
		...
		List<Integer> al = getListFromArray(arr);

		System.out.println(findDuplicatePositionInArray(arr));
		System.out.println(findDuplicatePositionInArray(al));
	}
	...
	private static int findDuplicatePositionInArray(List<Integer> al) {
		Map<Integer, Integer> countMap = new HashMap<>();
		for (int i = 0; i < al.size(); i++) {
			Integer number = countMap.get(al.get(i));
			if (number != null) {
				return al.get(i);
			} else {
				countMap.put(al.get(i), 1);
			}
		}
		return -1;
	}

	private static int findDuplicatePositionInArray(int[] arr) {
		Map<Integer, Integer> countMap = new HashMap<>();
		for (int i = 0; i < arr.length; i++) {
